...
# Default values
: ${NEL_X1:=40}
BUILD_DIR="build/${NEL_X1}"
BINARY="${BUILD_DIR}/main"

# 1. Build only if the binary doesn't exist (or force a build before the loop)
if [ ! -f "$BINARY" ]; then
    echo "Building project for NEL_X1=${NEL_X1}..."
    mkdir -p "$BUILD_DIR"
    cmake -B "$BUILD_DIR" -S .
    cmake --build "$BUILD_DIR" --parallel 8
fi

# 2. Machine detection (You were missing the CPU_MODEL definition in run_experiment.sh!)
